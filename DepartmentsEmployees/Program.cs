...
namespace DepartmentsEmployees
{
    class Program
    {
        ...
        public static void PrintDepartmentReport(string title, List<Department> departments)
        {
            ...
            departments.ForEach(el => Console.WriteLine($"{el.Id}: {el.DeptName}"));

            //foreach (var department in departments)
            //{
            //    Console.WriteLine($"{count}: {department.DeptName}");
            //    count++;
            //}
        }
        ...
        {
            Console.WriteLine(title);
            if (title.Equals("All Employees"))
            {
                int count = 1;
                foreach (var employee in employees)
                {
                    Console.WriteLine($" {count} {employee.FirstName} {employee.LastName}");
                    count++;
                }
            }
            else
            {
                ...
                foreach (var employee in employees)
                {
                    Console.WriteLine($" {count} {employee.FirstName} {employee.LastName} Dept: {employee.Department.DeptName}");
                    count++;
                }
            }
        }
